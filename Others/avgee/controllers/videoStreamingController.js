...
async function verifyLoginCredentials(req, res) {
  ...
  try {
    ...
    const { id } = user;
    await dbUpdateUserPasscodeInDatabase(id, newpasscode);

    user.passcode = newpasscode;
    // console.log("userwithnewpasscode: ", user);
    ...
    //redirect to login passcode page
    if (RES_FORMAT == "render") {
...
async function verifyLoginPasscodeCredentials(req, res) {
  ...
  let user = req.user;
  // console.log("userfromreq.user: ", user);

  //Get passcode from body
  const { passcode } = req.body;
  // console.log("passcodefrombody: ", passcode);
  ...
  //generate token with payload
  const homeToken = await new Promise((resolve, reject) => {
    jwt.sign(
      payload,
      PRIVATE_KEY,
      {
  ...
  // console.log("homeToken: ", homeToken);

  res.cookie("homeToken", homeToken, {
    httpOnly: true,
    secure: JWT_HTTPS,
    maxAge: JWT_EXPIRY,
  });
...
async function httpGetHomePage(req, res) {
  ...
  const context = {
    pagename: "HOME",
    title: "Avgee Career Master Class",
...
// #######################################
//  Error Controller - httpGetErrorPage
// #######################################

function httpGetErrorPage(req, res) {
  if (RES_FORMAT == "render") {
    return res.status(200).render("pages/errorlogin", { errors: [], msg: "" });
  }
  return res.status(200).json({});
}
